...
decimal grandAmount = 1999.99m;
DateTime enrolledAt = DateTime.UtcNow;
string? campusRegion = null;

Console.WriteLine($"Student: {studentName} ({studentId})");
Console.WriteLine($"Courses: {enrollmentCount}");
Console.WriteLine($"Grant: {grandAmount:F2}");
Console.WriteLine($"Enrolled: {enrolledAt:yyyy-MM-dd}");
Console.WriteLine($"Campus: {campusRegion ?? "Not assigned"}");


//  double cannot represent 0.99 exactly in binary
double grantDouble = 1999.99;
double totalDouble = grantDouble * 100_000;
Console.WriteLine($"Total allocated (double): {totalDouble:R}");


decimal grantPerStudent = 1999.99m;
decimal totalAllocation = grantPerStudent * 100000m;
...
// Exercise 4 Test IGradable polymorphism
...
Course[]  fetchedCourses  = await Task.WhenAll(courseTasks);

Console.WriteLine($"\nLoaded {fetchedStudents.Length} students and {fetchedCourses.Length} courses in {sw.ElapsedMilliseconds}ms");
foreach (var st in fetchedStudents)
    Console.WriteLine($"  {st.Name} GPA: {st.GPA}");
